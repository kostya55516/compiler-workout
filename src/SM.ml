...
let split n l =
  ...
  let rec print_list = 
  let prnt a = match a with
  | Value.Int n -> Printf.sprintf "%d" n
  | Value.String s -> s
  | Value.Array a -> print_list a
  in
  function 
  | [] -> ""
  | e::l -> Printf.sprintf "%s %s" (prnt e) (print_list l)
...
let rec eval env ((cs, stack, ((st, i, o) as c)) as cf) pl =
  ...
  match p with
  | BINOP op -> 
  if List.length stack < 2 then failwith (Printf.sprintf "op: %s stack: %s" op (print_list stack)) else
  let (y :: x :: stack) = stack in eval_d (cs, (Value.Int (Expr.to_func op (Value.to_int x) (Value.to_int y))) :: stack, c)
