...
let rec eval env ((cs, stack, ((st, i, o, r) as c)) as cf) pl =
  ...
  | CALL f        ->  let conf = (ps, st) :: cs, stack, c in
...
let compile (d, p) =
  ...
  | Expr.Call (f, exps)   -> List.fold_left (fun ins e -> (expr e) @ ins) [CALL (fun_label f)] exps
  in
  let rec state_comp =
  ...
  | Stmt.Call (f, exps) -> return (expr (Expr.Call (f, exps)))
  | Stmt.Return None    -> return [END]
  | Stmt.Return (Some e)-> return (expr e @ [END])
  in
